...
test(
  "creates and reuses a restricted local token",
  async (context) => {
...
test(
  ...
  async (context) => {
    ...
    await assert.rejects(
      ...
      (error: unknown) => {
        assert.ok(
          error instanceof LocalTokenStoreError
        );
...
test(
  "compares tokens without accepting different lengths",
  () => {
    const token = "a".repeat(64);

    assert.equal(
      secureTokenEqual(token, token),
      true
    );

    assert.equal(
      secureTokenEqual("b".repeat(64), token),
      false
    );

    assert.equal(
      secureTokenEqual("short", token),
      false
    );

    assert.equal(
      secureTokenEqual(undefined, token),
      false
    );
  }
);
