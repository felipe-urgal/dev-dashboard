...
function isValidProfileName(value: unknown): value is string {
  return (
    typeof value === 'string' &&
    value.trim().length > 0 &&
    value.length <= ENVIRONMENT_PROFILE_LIMITS.maxNameLength
  );
}

function sanitizeVariables(variables: unknown): EnvironmentProfileVariable[] {
  if (!Array.isArray(variables)) {
    throw new EnvironmentProfileRepositoryError(
      'ENVIRONMENT_PROFILE_INVALID',
      'A lista de variáveis é inválida.',
    );
  }
  if (variables.length > ENVIRONMENT_PROFILE_LIMITS.maxVariablesPerProfile) {
    throw new EnvironmentProfileRepositoryError(
      'ENVIRONMENT_PROFILE_INVALID',
      `O perfil aceita no máximo ${ENVIRONMENT_PROFILE_LIMITS.maxVariablesPerProfile} variáveis.`,
    );
  }
  ...
  for (const entry of variables) {
    if (typeof entry !== 'object' || entry === null) {
      throw new EnvironmentProfileRepositoryError(
        'ENVIRONMENT_PROFILE_INVALID',
        'Cada variável precisa ser um objeto.',
      );
    }
    ...
    if (typeof name !== 'string' || !VARIABLE_NAME_PATTERN.test(name)) {
      throw new EnvironmentProfileRepositoryError(
        'ENVIRONMENT_PROFILE_INVALID',
        'O nome de uma variável é inválido.',
      );
    }
    if (seen.has(name)) {
      throw new EnvironmentProfileRepositoryError(
        'ENVIRONMENT_PROFILE_INVALID',
        `A variável "${name}" está duplicada.`,
      );
    }
    seen.add(name);
    ...
    if (rawValue !== undefined && typeof rawValue !== 'string') {
      throw new EnvironmentProfileRepositoryError(
        'ENVIRONMENT_PROFILE_INVALID',
        `O valor da variável "${name}" é inválido.`,
      );
    }
    if (
      typeof rawValue === 'string' &&
      rawValue.length > ENVIRONMENT_PROFILE_LIMITS.maxValueLength
    ) {
      throw new EnvironmentProfileRepositoryError(
        'ENVIRONMENT_PROFILE_INVALID',
        `O valor da variável "${name}" excede o tamanho máximo.`,
      );
    }
...
function parseConfig(contents: string): EnvironmentProfile[] {
  ...
  if (typeof parsed !== 'object' || parsed === null || Array.isArray(parsed)) {
    throw new Error(
      'A configuração de perfis de ambiente possui formato inválido.',
    );
  }
  const candidate = parsed as Record<string, unknown>;
  if (candidate.version !== 1 || !Array.isArray(candidate.profiles)) {
    throw new Error(
      'A versão da configuração de perfis de ambiente não é suportada.',
    );
  }
  return (candidate.profiles as unknown[])
    .filter((entry): entry is EnvironmentProfile => {
      ...
      return (
        ...
        isValidProfileName(record.name) &&
  ...
  public async create(
    ...
    return this.enqueue(async () => {
      ...
      if (this.profiles.length >= ENVIRONMENT_PROFILE_LIMITS.maxProfiles) {
        throw new EnvironmentProfileRepositoryError(
          'ENVIRONMENT_PROFILE_LIMIT_REACHED',
          'O limite de perfis de ambiente foi atingido.',
        );
      }
  ...
  public async update(
    ...
    return this.enqueue(async () => {
      ...
      if (index === -1) {
        throw new EnvironmentProfileRepositoryError(
          'ENVIRONMENT_PROFILE_NOT_FOUND',
          'Perfil de ambiente não encontrado.',
        );
      }
      if (
        this.profiles.some(
