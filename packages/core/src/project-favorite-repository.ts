...
export class ProjectFavoriteRepositoryError extends Error {
  public constructor(
    public readonly code:
      'INVALID_PROJECT_ID' | 'PROJECT_FAVORITES_LIMIT_REACHED',
    message: string,
  ) {
    super(message);
...
function parseConfig(contents: string): Set<string> {
  ...
  if (candidate.version !== 1 || !Array.isArray(candidate.favoriteProjectIds)) {
...
export class ProjectFavoriteRepository {
  ...
  public async set(projectId: string, favorite: boolean): Promise<void> {
    ...
    const operation = this.mutationQueue.then(async () => {
      ...
      if (favorite) {
        if (!next.has(projectId) && next.size >= MAX_FAVORITES) {
          throw new ProjectFavoriteRepositoryError(
      ...
      await fsPromises.writeFile(
        ...
        `${JSON.stringify(
          ...
          } satisfies ProjectFavoriteConfig,
          null,
          2,
        )}\n`,
        {
