...
async function createFixture(): Promise<Fixture> {
  const root = await mkdtemp(path.join(tmpdir(), 'dev-dashboard-log-clear-'));
...
test('clears the persisted log and only shows subsequent output', async (context) => {
  const fixture = await createFixture();

  context.after(async () => {
    await fixture.manager.stopServer(fixture.project.id).catch(() => undefined);
  ...
  await waitForLog(fixture.manager, fixture.project.id, /before-clear/);

  const cleared = await fixture.manager.clearServerLog(fixture.project.id);

  assert.equal(cleared.content, '');
  assert.equal(cleared.sizeBytes, 0);

  const immediatelyAfterClear = await fixture.manager.readServerLog(
    fixture.project.id,
  );
...
test('disables the pnpm modules purge prompt for detached servers', async (context) => {
  const fixture = await createFixture();
...
test('forwards Next.js options without a double-dash when using pnpm', async (context) => {
  const fixture = await createFixture();
  const binDirectory = path.join(fixture.root, 'bin');
  const previousPath = process.env.PATH;
