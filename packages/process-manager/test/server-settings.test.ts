...
test(
  'persists an optional port per project',
  async (context) => {
...
test(
  'clears a previously configured port',
  async (context) => {
    const fixtureRoot = await mkdtemp(
...
test(
  'persists a valid relative health check path',
  async (context) => {
    const fixtureRoot = await mkdtemp(
...
test(
  'migrates legacy settings that contain a host',
  async (context) => {
    const fixtureRoot = await mkdtemp(
...
test(
  'ignores an unsafe health check path from persisted settings',
  async (context) => {
    const fixtureRoot = await mkdtemp(
