...
import { sweepStaleProcesses } from '../src/log-retention.js';
...
test('removes a stopped process older than the retention window', async (context) => {
...
test('keeps a stopped process within the retention window', async (context) => {
  const fixture = await createFixture();
  ...
  const { stateFilePath, logPath } = await writeStateFile(
    ...
    'recent.server.json',
    {
      ...
      args: ['run', 'dev'],
      cwd: fixture.stateDirectory,
      logPath: path.join(fixture.processDirectory, 'recent.server.json.log'),
    },
  );
...
test('removes a recent stopped process when all terminal states are requested', async (context) => {
  const fixture = await createFixture();
  ...
  assert.deepEqual(removed, [{ projectId: 'recent-terminal-project' }]);
  await assert.rejects(stat(stateFilePath));
  await assert.rejects(stat(logPath));
});

test('removes a failed process older than the retention window', async (context) => {
  const fixture = await createFixture();
  ...
  await writeStateFile(fixture, 'failed.server.json', {
    ...
    args: ['run', 'dev'],
    cwd: fixture.stateDirectory,
    logPath: path.join(fixture.processDirectory, 'failed.server.json.log'),
  });
...
test("falls back to the state file's mtime when stoppedAt is missing", async (context) => {
  const fixture = await createFixture();
...
test('returns an empty list when the processes directory does not exist', async (context) => {
  const fixtureRoot = await mkdtemp(
  ...
  const removed = await sweepStaleProcesses(path.join(fixtureRoot, 'state'));

  assert.deepEqual(removed, []);
});

test('skips a corrupt state file and still removes a valid stale process', async (context) => {
  const fixture = await createFixture();
...
test('removes an orphaned log older than the retention window', async (context) => {
  const fixture = await createFixture();

  context.after(fixture.cleanup);

  const orphanLogPath = path.join(fixture.logDirectory, 'orphan.server.log');
...
test('keeps a recent orphaned log within the retention window', async (context) => {
  const fixture = await createFixture();
...
test('removes a recent orphaned log when all terminal states are requested', async (context) => {
  const fixture = await createFixture();

  context.after(fixture.cleanup);

  const orphanLogPath = path.join(fixture.logDirectory, 'orphan.test.log');
...
test('keeps a log that still has a matching state file, even if invalid', async (context) => {
  const fixture = await createFixture();

  context.after(fixture.cleanup);

  const logPath = path.join(fixture.logDirectory, 'garbage.server.log');
...
test('removes a process marked running whose PID has already died', async (context) => {
  const fixture = await createFixture();
  ...
  const { stateFilePath } = await writeStateFile(fixture, 'dead.server.json', {
    id: 'dead:server',
    ...
    args: ['run', 'dev'],
    cwd: fixture.stateDirectory,
    logPath: path.join(fixture.processDirectory, 'dead.server.json.log'),
  });
...
test('keeps a process that is genuinely running when all terminal states are requested', async (context) => {
  const fixture = await createFixture();

  // eslint-disable-next-line prefer-const -- context.after fecha sobre esta variável antes da atribuição; `const` causaria TDZ se o after rodar antes.
  ...
  const { stateFilePath } = await writeStateFile(fixture, 'alive.server.json', {
    id: 'alive:server',
    ...
    args: ['run', 'dev'],
    cwd: projectCwd,
    logPath: path.join(fixture.processDirectory, 'alive.server.json.log'),
  });
...
test('never removes a log path outside the managed logs directory', async (context) => {
  const fixture = await createFixture();
