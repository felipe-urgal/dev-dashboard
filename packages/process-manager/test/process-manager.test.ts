...
async function configureHttpServer(fixture: Fixture): Promise<void> {
  await Promise.all([
    writeFile(
...
test('returns null when no process was ever started for a project', async (context) => {
...
test('rejects reading logs without a stored process', async (context) => {
  const fixture = await createFixture({
    name: 'fixture',
    scripts: {},
  });
...
test('rejects starting a server for an unsupported project type', async (context) => {
  const fixture = await createFixture({
    name: 'fixture',
    scripts: {},
  });
...
test('rejects starting a node server without a dev, start or serve script', async (context) => {
  const fixture = await createFixture({
    name: 'fixture',
...
test('rejects starting a server on an out-of-range port without spawning anything', async (context) => {
  const fixture = await createFixture({
    name: 'fixture',
    scripts: { dev: 'node -e "setInterval(() => {}, 60000)"' },
  });
...
test('starts, tracks and stops a real node server', async (context) => {
  const fixture = await createFixture({
    name: 'fixture',
    scripts: { dev: 'node -e "setInterval(() => {}, 60000)"' },
  });
  ...
  const stoppedAgain = await fixture.manager.stopServer(fixture.project.id);

  assert.equal(stoppedAgain.status, 'stopped');
});

test('captures server stdout in the log file', async (context) => {
  const fixture = await createFixture({
    name: 'fixture',
    scripts: {
  ...
  const snapshot = await fixture.manager.readServerLog(fixture.project.id);
...
test('truncates long logs and drops the leading partial line', async (context) => {
  const fixture = await createFixture({
    name: 'fixture',
    scripts: {
      dev:
...
test('rejects an out-of-range log limit for a known process', async (context) => {
  const fixture = await createFixture({
    name: 'fixture',
    scripts: { dev: 'node -e "setInterval(() => {}, 60000)"' },
  });
...
test('detects a process that exits during startup and marks it failed', async (context) => {
  const fixture = await createFixture({
    name: 'fixture',
    scripts: {
  ...
  const detected = await fixture.manager.getServerProcess(fixture.project.id);
...
test('records a non-zero exit after the server was running', async (context) => {
  const fixture = await createFixture({
    name: 'fixture',
    scripts: { dev: 'node server.js' },
  });
...
test('starts a server even when the log sweep finds a corrupted state file', async (context) => {
  const fixture = await createFixture({
    name: 'fixture',
    scripts: { dev: 'node -e "setInterval(() => {}, 60000)"' },
  });
...
test('serializes concurrent startTest invocations', async (context) => {
  const fixture = await createFixture({
    name: 'fixture',
...
test('rejects a configured port that is already occupied', async (context) => {
  const fixture = await createFixture({
    name: 'fixture',
    scripts: { dev: 'node -e "setInterval(() => {}, 60000)"' },
  });
...
test('starts, reports and stops a worker process (sidekiq-like)', async (context) => {
  const fixture = await createFixture({
    name: 'fixture',
    scripts: { dev: 'node -e "setInterval(() => {}, 60000)"' },
  });
...
test('keeps sidekiq and webpack workers as independent processes for the same project', async (context) => {
  const fixture = await createFixture({
    name: 'fixture',
    scripts: { dev: 'node -e "setInterval(() => {}, 60000)"' },
  });
...
test('rejects starting a worker that is already running', async (context) => {
  const fixture = await createFixture({
    name: 'fixture',
    scripts: { dev: 'node -e "setInterval(() => {}, 60000)"' },
  });
  ...
  context.after(async () => {
    await fixture.manager
      .stopWorker(fixture.project.id, 'worker')
      .catch(() => undefined);
  });
