...
export function createCommitOperations(confirmations: GitMutationConfirmationService) {
  async function commit(projectPath: string, projectId: string, message: string, includeAllChanges: boolean, confirmationToken?: string): Promise<GitCommitResult> {
    validateCommitMessage(message);
    await requireRepository(projectPath);
    const status = parseStatus(await runGit(projectPath, ['status', '--porcelain=v2', '--branch', '-z', '--untracked-files=all']));
    const branch = status.branch ?? 'HEAD';
    consumeMutationConfirmation(confirmations, projectId, 'commit', branch, confirmationToken);
  ...
  async function amend(projectPath: string, projectId: string, message: string, confirmationToken?: string): Promise<GitCommitResult> {
    validateCommitMessage(message);
    await requireRepository(projectPath);
    const status = parseStatus(await runGit(projectPath, ['status', '--porcelain=v2', '--branch', '-z', '--untracked-files=all']));
    const branch = status.branch ?? 'HEAD';
    consumeMutationConfirmation(confirmations, projectId, 'amend', branch, confirmationToken);
    try {
  ...
  async function save(projectPath: string, projectId: string, message: string, confirmationToken?: string): Promise<GitCommitResult> {
    validateCommitMessage(message);
    await requireRepository(projectPath);
    const status = parseStatus(await runGit(projectPath, ['status', '--porcelain=v2', '--branch', '-z', '--untracked-files=all']));
    const branch = status.branch ?? 'HEAD';
    consumeMutationConfirmation(confirmations, projectId, 'save', branch, confirmationToken);
