...
import { EVENT_THROTTLE_MS, MAX_EXECUTION_SUBSCRIBERS, MAX_TOTAL_SUBSCRIBERS } from './constants.js';
import { ScriptExecutionError } from './errors.js';
import { getExecution, readExecutionLog } from './store.js';
import type { ExecutionSubscriber, RunningExecution, ScriptExecutionContext } from './state.js';
...
export async function subscribeToExecution(
  ...
  const current = context.subscribers.get(executionId) ?? new Set<ExecutionSubscriber>();
  const total = Array.from(context.subscribers.values()).reduce((sum, items) => sum + items.size, 0);
  ...
  try {
    ...
    subscriber.send({ type: 'log', log: await readExecutionLog(context, projectId, executionId) });
  } catch (error) {
...
export function emitEvent(context: ScriptExecutionContext, record: RunningExecution, event: ScriptExecutionEvent): void {
  for (const subscriber of context.subscribers.get(record.execution.id) ?? []) subscriber.send(event);
...
export function scheduleLogEvent(context: ScriptExecutionContext, record: RunningExecution, immediate = false): void {
  ...
  const publish = (): void => {
    ...
    void readExecutionLog(context, record.execution.projectId, record.execution.id)
      .then((log) => emitEvent(context, record, { type: 'log', log }))
      .catch(() => undefined);
  };
  if (immediate) queueMicrotask(publish);
  else context.eventTimers.set(record.execution.id, setTimeout(publish, EVENT_THROTTLE_MS));
}
