...
interface SnapshotListResponse {
  snapshots: {
    snapshots: Array<{ id: string; database: string; label: string; sizeBytes: number }>;
    total: number;
    retentionLimit: number;
    supportedEnvironmentIds: string[];
  };
}
interface SnapshotResponse { snapshot: { id: string; database: string; driver: string } }
interface ConfirmationResponse { confirmation: { token: string; snapshotId: string; expiresAt: string } }
interface RestoreResponse { restore: { snapshotId: string; restored: boolean } }
interface ErrorResponse { error?: string }

test('rotas de snapshot e restore do banco', async (context) => {
  const fixtureRoot = await mkdtemp(path.join(tmpdir(), 'dev-dashboard-db-snapshot-routes-'));
  ...
  await writeFile(path.join(binDirectory, 'pg_dump'), '#!/usr/bin/env bash\necho "SELECT 1;"\n');
  await writeFile(path.join(binDirectory, 'psql'), `#!/usr/bin/env bash\ncat > ${JSON.stringify(restoreLog)}\n`);
  ...
  await context.test('exige o token nas rotas de snapshot', async () => {
    const response = await app.inject({ method: 'GET', url: '/api/projects/p1/database/snapshots' });
    assert.equal(response.statusCode, 401);
  });

  await context.test('lista vazia informa retenção e ambientes compatíveis', async () => {
    const response = await app.inject({ method: 'GET', url: '/api/projects/p1/database/snapshots', headers });
  ...
  await context.test('cria um snapshot para o ambiente detectado', async () => {
    const response = await app.inject({
      method: 'POST', url: '/api/projects/p1/database/snapshots', headers,
  ...
  await context.test('ignora campos de conexão enviados pelo navegador', async () => {
    ...
    const response = await app.inject({
      method: 'POST', url: '/api/projects/p1/database/snapshots', headers,
      payload: { environmentId: 'dotenv--env', host: 'servidor-externo', database: 'outro' },
    });
    assert.equal(response.statusCode, 200);
    assert.equal(response.json<SnapshotResponse>().snapshot.database, 'app_development');
  });

  await context.test('recusa id de snapshot fora do formato UUID', async () => {
    const response = await app.inject({
      method: 'POST', url: '/api/projects/p1/database/snapshots/..%2Fescape/restore/confirmation', headers, payload: {},
    });
  ...
  await context.test('restore sem confirmação válida é recusado', async () => {
    const response = await app.inject({
      method: 'POST', url: `/api/projects/p1/database/snapshots/${snapshotId}/restore`, headers,
      payload: { confirmationToken: 'f'.repeat(64) },
    });
    assert.equal(response.statusCode, 409);
    assert.equal(response.json<ErrorResponse>().error, 'DATABASE_RESTORE_CONFIRMATION_REQUIRED');
  });

  await context.test('restaura em duas etapas e devolve apenas metadados', async () => {
    const prepared = await app.inject({
      method: 'POST', url: `/api/projects/p1/database/snapshots/${snapshotId}/restore/confirmation`, headers, payload: {},
    });
    ...
    assert.deepEqual(restored.json<RestoreResponse>().restore, { snapshotId, restored: true });
  });

  await context.test('a listagem nunca expõe caminho de arquivo do dump', async () => {
    const response = await app.inject({ method: 'GET', url: '/api/projects/p1/database/snapshots', headers });
    const { snapshots } = response.json<SnapshotListResponse>();
  ...
  await context.test('projeto inexistente responde 404', async () => {
    const response = await app.inject({ method: 'GET', url: '/api/projects/desconhecido/database/snapshots', headers });
