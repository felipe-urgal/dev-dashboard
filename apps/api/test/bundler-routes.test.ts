...
test('rota de diagnóstico Bundler', async (context) => {
  const fixtureRoot = await mkdtemp(
    path.join(tmpdir(), 'dev-dashboard-bundler-routes-'),
  );
  ...
  const { BundlerInspectionService } =
    await import('../src/services/bundler-inspection-service.js');

  const appContext = createAppContext();
  appContext.bundlerInspectionService = new BundlerInspectionService(
    async (_command, args) => {
      if (args[0] === 'check')
        return { stdout: "The Gemfile's dependencies are satisfied\n" };
      return {
        stdout: '  * puma (newest 6.4.2, installed 6.4.0, requested ~> 6.4)\n',
      };
    },
  );
  ...
  await context.test('retorna diagnóstico Bundler', async () => {
    ...
    assert.deepEqual(bundler.outdated, [
      {
        name: 'puma',
        installed: '6.4.0',
        newest: '6.4.2',
  ...
  await context.test('retorna 404 para projeto inexistente', async () => {
    const response = await app.inject({
      method: 'GET',
      url: '/api/projects/does-not-exist/bundler',
      headers,
    });
