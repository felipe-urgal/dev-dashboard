...
test('histórico de mutações Git: gravação por operação, paginação e autenticação', async (context) => {
  const fixtureRoot = await mkdtemp(
    path.join(tmpdir(), 'dev-dashboard-git-mutation-history-routes-'),
  );
  ...
  await context.test(
    'criar branch com sucesso é registrada no histórico com risco do catálogo',
    async () => {
  ...
  await context.test(
    'falha controlada na execução é registrada com o código de erro',
    async () => {
      const confirmationResponse = await app.inject({
        method: 'POST',
        url: '/api/projects/p1/git/mutations/confirmations',
      ...
      const failure = body.events.find(
        (event) => event.operationId === 'switch-branch',
      );
      assert.ok(failure);
  ...
  await context.test(
    'paginação respeita page/pageSize e ordena do mais recente',
    async () => {
      ...
      const pageOne = await app.inject({
        method: 'GET',
        url: '/api/projects/p1/git/mutation-history?page=1&pageSize=2',
        headers,
      });
  ...
  await context.test('rota rejeita projeto desconhecido com 404', async () => {
    const response = await app.inject({
      method: 'GET',
      url: '/api/projects/ghost/git/mutation-history',
      headers,
    });
    assert.equal(response.statusCode, 404);
