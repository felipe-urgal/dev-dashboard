...
import { registerStaticDashboard, validateDashboardBuild } from '../src/http/static-dashboard.js';

async function fixture() {
  const root = await mkdtemp(path.join(tmpdir(), 'dashboard-static-'));
  await mkdir(path.join(root, 'assets'));
  await writeFile(path.join(root, 'index.html'), '<script src="/assets/app-abc123.js"></script>');
...
test('serve HTML, asset versionado e fallback com caches distintos', async (context) => {
  const root = await fixture(); const app = Fastify({ logger: false });
  app.get('/api/known', async () => ({ ok: true }));
  await registerStaticDashboard(app, root);
  context.after(async () => { await app.close(); await rm(root, { recursive: true }); });
  const html = await app.inject({ url: '/', headers: { accept: 'text/html' } });
  assert.equal(html.statusCode, 200); assert.equal(html.headers['cache-control'], 'no-cache');
  ...
  assert.equal((await app.inject({ url: '/projects/um', headers: { accept: 'text/html' } })).statusCode, 200);
  assert.equal((await app.inject({ url: '/assets/missing.js', headers: { accept: 'text/html' } })).statusCode, 404);
  assert.equal((await app.inject({ method: 'POST', url: '/projects/um', headers: { accept: 'text/html' } })).statusCode, 404);
  assert.equal((await app.inject({ url: '/api/missing', headers: { accept: 'text/html' } })).statusCode, 404);
  assert.equal((await app.inject({ url: '/api/known', headers: { accept: 'text/html' } })).json().ok, true);
});
...
test('falha para build ausente, sem index ou sem asset obrigatório', async () => {
  ...
  await assert.rejects(validateDashboardBuild(path.join(root, 'ausente')), /diretório/);
