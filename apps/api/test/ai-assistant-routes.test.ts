...
test('rotas do assistente de IA (status e chat em streaming)', async (context) => {
  const fixtureRoot = await mkdtemp(path.join(tmpdir(), 'dev-dashboard-ai-routes-'));
  ...
  const { ProjectFileService } = await import('../src/services/project-file-service.js');
  const { GitService } = await import('../src/services/git-service.js');

  const appContext = createAppContext();
  const project: Project = {
    id: 'p1', name: 'sample', path: fixtureRoot,
  ...
  appContext.aiAssistantService = new AiAssistantService(
    ...
    async (input) => {
      ...
      if (url.endsWith('/api/chat')) {
        const body = [
          JSON.stringify({ message: { role: 'assistant', content: 'Oi' }, done: false }),
  ...
  await context.test('status retorna modelos instalados', async () => {
    ...
    assert.deepEqual(status.models, [{ name: 'llama3.1', capabilities: ['chat'] }]);
  });
  ...
  await context.test('chat transmite eventos SSE com a resposta do modelo', async () => {
    const response = await app.inject({
      method: 'POST',
      url: '/api/projects/p1/ai/chat',
      headers: { ...headers, 'content-type': 'application/json' },
      payload: { model: 'llama3.1', messages: [{ role: 'user', content: 'Oi' }] },
    });
    assert.equal(response.statusCode, 200);
  ...
  await context.test('rota exige autenticação', async () => {
    const response = await app.inject({ method: 'GET', url: '/api/projects/p1/ai/status' });
