...
import { EnvironmentProfileRepository, RetentionSettingsRepository } from '@dev-dashboard/core';
...
test('cria, atualiza, lista e remove perfis de ambiente sem persistir valores sensíveis', async () => {
  const context = createAppContext();
  context.environmentProfileRepository = new EnvironmentProfileRepository(await mkdtemp(path.join(tmpdir(), 'api-environment-profiles-')));
  const app = await buildApp({ localToken: token, context });
  const headers = { 'x-dev-dashboard-token': token };

  const unauthorized = await app.inject({ method: 'GET', url: '/api/settings/environment-profiles' });
  assert.equal(unauthorized.statusCode, 401);

  const created = await app.inject({
    method: 'POST', url: '/api/settings/environment-profiles', headers,
    payload: { name: 'Staging', variables: [{ name: 'API_URL', value: 'https://staging.example.com' }, { name: 'API_SECRET_TOKEN', value: 'segredo' }] },
  });
  assert.equal(created.statusCode, 201);
  const profile = created.json().profile;
  assert.deepEqual(profile.variables, [{ name: 'API_URL', value: 'https://staging.example.com' }, { name: 'API_SECRET_TOKEN' }]);

  const listed = await app.inject({ method: 'GET', url: '/api/settings/environment-profiles', headers });
  assert.equal(listed.statusCode, 200);
  assert.equal(listed.json().profiles.length, 1);
  assert.equal(listed.json().limits.maxProfiles, 50);

  const updated = await app.inject({
    method: 'PUT', url: `/api/settings/environment-profiles/${profile.id}`, headers,
    payload: { name: 'Staging 2', variables: [{ name: 'PORT', value: '4000' }] },
  });
  ...
  const removed = await app.inject({ method: 'DELETE', url: `/api/settings/environment-profiles/${profile.id}`, headers });
  assert.equal(removed.statusCode, 204);

  const notFound = await app.inject({ method: 'DELETE', url: `/api/settings/environment-profiles/${profile.id}`, headers });
