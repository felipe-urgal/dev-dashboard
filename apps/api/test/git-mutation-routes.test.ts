...
test('rotas de mutação Git: confirmação, criação e troca de branch', async (context) => {
  const fixtureRoot = await mkdtemp(
    path.join(tmpdir(), 'dev-dashboard-git-routes-'),
  );
  ...
  await context.test('confirmar + criar branch retorna 201', async () => {
    ...
    const createResponse = await app.inject({
      method: 'POST',
      url: '/api/projects/p1/git/branches',
      headers,
      payload: JSON.stringify({
        name: 'feature/route',
    ...
    assert.equal(
      createResponse.json<BranchResponse>().branch.branch,
      'feature/route',
    );
  });

  await context.test(
    'criar branch sem confirmação retorna 409 GIT_MUTATION_CONFIRMATION_REQUIRED',
    async () => {
  ...
  await context.test(
    'trocar para branch inexistente retorna 404 GIT_BRANCH_NOT_FOUND',
    async () => {
      const confirmationResponse = await app.inject({
        method: 'POST',
        url: '/api/projects/p1/git/mutations/confirmations',
      ...
      const response = await app.inject({
        method: 'POST',
        url: '/api/projects/p1/git/switch',
        headers,
        payload: JSON.stringify({
          name: 'no-such-branch',
          confirmationToken: confirmation.token,
  ...
  await context.test('rota rejeita projeto desconhecido com 404', async () => {
    const response = await app.inject({
      method: 'POST',
      url: '/api/projects/ghost/git/mutations/confirmations',
      headers,
  ...
  await context.test(
    'push em repositório sem remoto retorna 409 GIT_REMOTE_NOT_CONFIGURED',
    async () => {
  ...
  await context.test(
    'pull sem upstream retorna 409 GIT_NO_UPSTREAM',
    async () => {
      const confirmationResponse = await app.inject({
        method: 'POST',
        url: '/api/projects/p1/git/mutations/confirmations',
  ...
  await context.test(
    'pull/push sem confirmação retornam 409 GIT_MUTATION_CONFIRMATION_REQUIRED',
    async () => {
      ...
      assert.equal(
        pullResponse.json<ErrorResponse>().error,
        'GIT_MUTATION_CONFIRMATION_REQUIRED',
      );
      ...
      assert.equal(
        pushResponse.json<ErrorResponse>().error,
        'GIT_MUTATION_CONFIRMATION_REQUIRED',
      );
    },
  );
  ...
  await context.test(
    'pull/push em projeto desconhecido retornam 404',
    async () => {
      const pullResponse = await app.inject({
        method: 'POST',
        url: '/api/projects/ghost/git/pull',
        headers,
        payload: JSON.stringify({ confirmationToken: 'z'.repeat(64) }),
      });
      ...
      const pushResponse = await app.inject({
        method: 'POST',
        url: '/api/projects/ghost/git/push',
        headers,
        payload: JSON.stringify({ confirmationToken: 'z'.repeat(64) }),
      });
  ...
  await context.test(
    'commit sem nada staged retorna 409 GIT_NOTHING_TO_COMMIT',
    async () => {
      const confirmationResponse = await app.inject({
        method: 'POST',
        url: '/api/projects/p1/git/mutations/confirmations',
  ...
  await context.test('amend altera a mensagem do último commit', async () => {
    ...
    const response = await app.inject({
      method: 'POST',
      url: '/api/projects/p1/git/commit/amend',
      headers,
      payload: JSON.stringify({
  ...
  await context.test(
    'save via rota prepara tudo e aplica o prefixo do branch',
    async () => {
      ...
      const response = await app.inject({
        method: 'POST',
        url: '/api/projects/p1/git/save',
        headers,
        payload: JSON.stringify({
          message: 'salva tudo via rota',
          confirmationToken: confirmation.token,
        }),
      });
      assert.equal(response.statusCode, 201);
      ...
      assert.equal(
        response.json<SaveResponse>().commit.subject,
        'fix: salva tudo via rota',
      );
    },
  );

  await context.test(
    'save com árvore limpa retorna 409 GIT_NOTHING_TO_COMMIT',
    async () => {
      const confirmationResponse = await app.inject({
        method: 'POST',
        url: '/api/projects/p1/git/mutations/confirmations',
  ...
  await context.test(
    'save sem confirmação retorna 409 GIT_MUTATION_CONFIRMATION_REQUIRED',
    async () => {
      const response = await app.inject({
        method: 'POST',
        url: '/api/projects/p1/git/save',
