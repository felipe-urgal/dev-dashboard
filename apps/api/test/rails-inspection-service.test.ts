...
test('executa migrate após confirmação válida, com o comando esperado', async () => {
  ...
  const confirmation = await service.prepareMutationConfirmation(
    project,
    'migrate',
  );
  const result = await service.runMutation(
    project,
    'migrate',
    confirmation.token,
  );
  ...
  assert.deepEqual(calls, [
    { command: path.join(project.path, 'bin', 'rails'), args: ['db:migrate'] },
  ]);
});
...
test('rejeita execução sem confirmação', async () => {
  ...
  await assert.rejects(
    ...
    (error: unknown) =>
      ...
      error.code === 'RAILS_MUTATION_CONFIRMATION_REQUIRED',
  );
});
...
test('mascara segredos na saída da mutação', async () => {
  ...
  const service = new RailsInspectionService(async () => ({
    stdout: 'DATABASE_URL=postgresql://user:senha@localhost/app\n',
  }));
