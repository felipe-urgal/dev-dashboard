...
async function fixture(type: Project['type']): Promise<Project> {
  const directory = await mkdtemp(path.join(os.tmpdir(), 'dashboard-dependencies-'));
  return {
...
test('detecta Yarn e expõe instalação e build com os comandos corretos', async () => {
  ...
  try {
    await writeFile(path.join(project.path, 'package.json'), JSON.stringify({
    ...
    const install = catalog.items.find((item) => item.id === 'package-manager:install');
    const build = catalog.items.find((item) => item.id === 'package-script:build');
...
test('projeto Rails com frontend recebe ações Bundler e Node', async () => {
  ...
  try {
    ...
    assert.equal(
      catalog.items.find((item) => item.id === 'package-manager:install')?.command,
      'npm install',
    );
    assert.equal(
