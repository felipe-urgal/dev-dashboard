...
test('histórico de mutações Git: sincronização (sync-integrate/sync-main)', async (context) => {
  const fixtureRoot = await mkdtemp(
    path.join(tmpdir(), 'dev-dashboard-git-sync-history-'),
  );
  ...
  await context.test(
    'integração bem-sucedida é registrada como sync-integrate com risco write-remote',
    async () => {
      const confirmationResponse = await app.inject({
        method: 'POST',
        url: '/api/projects/p1/git/sync/confirmations',
        headers,
  ...
  await context.test(
    'confirmação ausente não gera evento de histórico',
    async () => {
  ...
  await context.test(
    'falha controlada na sincronização é registrada com o código de erro',
    async () => {
      const confirmationResponse = await app.inject({
        method: 'POST',
        url: '/api/projects/p1/git/sync/confirmations',
        headers,
        payload: JSON.stringify({
          reference: 'upstream/no-such-branch',
          strategy: 'ff-only',
        }),
      });
      const { confirmation } =
      ...
      const failure = body.events.find(
        (event) => event.errorCode === 'GIT_REFERENCE_NOT_FOUND',
      );
      assert.ok(failure);
  ...
  await context.test(
    'sincronização da main é registrada como sync-main',
    async () => {
      ...
      const confirmationResponse = await app.inject({
        method: 'POST',
        url: '/api/projects/p1/git/sync/main/confirmations',
      ...
      const response = await app.inject({
        method: 'POST',
        url: '/api/projects/p1/git/sync/main',
        headers,
      ...
      const body = historyResponse.json<HistoryResponse>();
      const success = body.events.find(
        (event) => event.operationId === 'sync-main',
      );
      assert.ok(success);
