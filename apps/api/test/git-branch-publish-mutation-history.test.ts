...
/**
 * Verifica que `GitBranchPublishService` (`branch-publish`), migrado da task
 * 098, grava eventos no histórico compartilhado no mesmo padrão de
 * `git-mutation-history-routes.test.ts`.
 */
test('histórico de mutações Git: publicar branch local (branch-publish)', async (context) => {
  const fixtureRoot = await mkdtemp(path.join(tmpdir(), 'dev-dashboard-git-publish-history-'));
  const originPath = path.join(fixtureRoot, 'origin.git');
  const repoPath = path.join(fixtureRoot, 'sample');
  await execFileAsync('git', ['init', '--bare', '-q', '-b', 'main', originPath]);
  ...
  await git(repoPath, ['switch', '--create', 'feature/publish-me']);
  ...
  await context.test('publicar com sucesso é registrado com risco write-remote', async () => {
    const confirmationResponse = await app.inject({
      method: 'POST', url: '/api/projects/p1/git/branches/publish/confirmations', headers,
      payload: JSON.stringify({ branch: 'feature/publish-me' }),
    });
    const { confirmation } = confirmationResponse.json<ConfirmationResponse>();
    const publishResponse = await app.inject({
      method: 'POST', url: '/api/projects/p1/git/branches/publish', headers,
      payload: JSON.stringify({ branch: 'feature/publish-me', confirmationToken: confirmation.token }),
    });
    ...
    assert.equal(body.events[0]!.operationId, 'branch-publish');
  ...
  await context.test('confirmação ausente/expirada não gera evento de histórico', async () => {
    ...
    const response = await app.inject({
      method: 'POST', url: '/api/projects/p1/git/branches/publish', headers,
      payload: JSON.stringify({ branch: 'feature/publish-me', confirmationToken: 'z'.repeat(64) }),
    });
  ...
  await context.test('falha controlada (branch inexistente) é registrada com o código de erro', async () => {
    const confirmationResponse = await app.inject({
      method: 'POST', url: '/api/projects/p1/git/branches/publish/confirmations', headers,
      payload: JSON.stringify({ branch: 'no-such-branch' }),
    });
    const { confirmation } = confirmationResponse.json<ConfirmationResponse>();
    const response = await app.inject({
      method: 'POST', url: '/api/projects/p1/git/branches/publish', headers,
      payload: JSON.stringify({ branch: 'no-such-branch', confirmationToken: confirmation.token }),
    });
    ...
    const historyResponse = await app.inject({ method: 'GET', url: '/api/projects/p1/git/mutation-history', headers });
    const body = historyResponse.json<HistoryResponse>();
    const failure = body.events.find((event) => event.errorCode === 'GIT_BRANCH_NOT_FOUND');
    assert.ok(failure);
    assert.equal(failure!.operationId, 'branch-publish');
