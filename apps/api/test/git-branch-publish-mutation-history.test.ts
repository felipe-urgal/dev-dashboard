...
test('histórico de mutações Git: publicar branch local (branch-publish)', async (context) => {
  const fixtureRoot = await mkdtemp(
    path.join(tmpdir(), 'dev-dashboard-git-publish-history-'),
  );
  ...
  await context.test(
    'publicar com sucesso é registrado com risco write-remote',
    async () => {
      const confirmationResponse = await app.inject({
        method: 'POST',
        url: '/api/projects/p1/git/branches/publish/confirmations',
        headers,
      ...
      const publishResponse = await app.inject({
        method: 'POST',
        url: '/api/projects/p1/git/branches/publish',
        headers,
        payload: JSON.stringify({
          branch: 'feature/publish-me',
