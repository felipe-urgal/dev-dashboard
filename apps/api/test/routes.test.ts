...
test('handles workspace and process route errors consistently', async (context) => {
  const fixtureRoot = await mkdtemp(
    path.join(tmpdir(), 'dev-dashboard-api-routes-'),
  );
  ...
  await context.test(
    ...
    async () => {
      ...
      assert.equal(
        body.directories.some((directory) => directory.path === workspacePath),
        true,
      );
    },
  );
  ...
  await context.test(
    ...
    async () => {
      const missingPath = path.join(fixtureRoot, 'missing-directory');
  ...
  await context.test(
    ...
    async () => {
      const configuredRoot = process.env.DEV_DASHBOARD_DIRECTORY_ROOT;
      ...
      try {
        ...
        assert.equal(response.json<JsonResponse>().error, 'INVALID_DIRECTORY');
      } finally {
  ...
  await context.test('validates workspace payloads', async () => {
    const response = await app.inject({
      method: 'POST',
      url: '/api/workspaces',
      headers,
  ...
  await context.test(
    ...
    async () => {
      ...
      const listBody = listResponse.json<JsonResponse>();
      ...
      const duplicateBody = duplicateResponse.json<JsonResponse>();

      assert.equal(duplicateResponse.statusCode, 409);

      assert.equal(duplicateBody.error, 'WORKSPACE_ALREADY_EXISTS');
    },
  );
  ...
  await context.test(
    ...
    async () => {
      const missingPath = path.join(fixtureRoot, 'missing-workspace');
      ...
      assert.equal(response.statusCode, 400);
      assert.equal(body.error, 'WORKSPACE_CREATION_FAILED');

      assert.equal(response.body.includes(missingPath), false);
    },
  );

  await context.test('returns not found for an unknown workspace', async () => {
    const response = await app.inject({
      method: 'POST',
  ...
  await context.test(
    ...
    async () => {
      ...
      assert.equal(scanBody.projects[0]?.name, 'sample-node');
      ...
      const projectsBody = projectsResponse.json<JsonResponse>();
  ...
  await context.test(
    ...
    async () => {
      ...
      assert.equal(
        projectsResponse.json<JsonResponse>().projects?.[0]?.favorite,
        true,
      );
    },
  );

  await context.test('persists project server port settings', async () => {
  ...
  await context.test('maps real ProcessManager errors', async () => {
    ...
    const statusBody = statusResponse.json<JsonResponse>();
    ...
    const logsBody = logsResponse.json<JsonResponse>();

    assert.equal(logsResponse.statusCode, 404);
    assert.equal(logsBody.error, 'PROCESS_NOT_FOUND');
    ...
    const clearLogsBody = clearLogsResponse.json<JsonResponse>();

    assert.equal(clearLogsResponse.statusCode, 404);
    assert.equal(clearLogsBody.error, 'PROCESS_NOT_FOUND');
    ...
    const startBody = startResponse.json<JsonResponse>();

    assert.equal(startResponse.statusCode, 400);
    assert.equal(startBody.error, 'PROJECT_SCRIPT_NOT_FOUND');
    ...
    const stopBody = stopResponse.json<JsonResponse>();

    assert.equal(stopResponse.statusCode, 404);
    assert.equal(stopBody.error, 'PROCESS_NOT_FOUND');
  });
  ...
  await context.test(
    ...
    async () => {
      const nestedProjectPath = path.join(workspacePath, 'apps', 'nested-node');
      ...
      const patchMissingBody = patchMissingResponse.json<JsonResponse>();

      assert.equal(patchMissingResponse.statusCode, 404);
      assert.equal(patchMissingBody.error, 'WORKSPACE_NOT_FOUND');
    },
  );

  await context.test('removes the workspace', async () => {
    const response = await app.inject({
      method: 'DELETE',
      url: `/api/workspaces/${workspaceId}`,
      headers,
