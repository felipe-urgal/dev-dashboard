...
function reviewFilesService() {
  return {
    ...
    getReviewFileDiff: async (_path: string, _input: unknown, filePath: string) => ({
      targetRemote: 'origin' as const,
      baseBranch: 'main',
      sourceBranch: 'feature/sintese',
      files: ['src/a.ts', 'src/b.ts'],
  ...
  const service = new GitAiCodeReviewService(
    {
      review: async (_model, messages) => {
        ...
        return localReview(prompt.includes('src/a.ts') ? 'src/a.ts' : 'src/b.ts');
      },
...
test('complete executa síntese global e deduplica achados equivalentes', async () => {
  let calls = 0;
  let synthesisPrompt = '';
  const service = new GitAiCodeReviewService(
    {
...
test('complete falha explicitamente quando a síntese global não retorna JSON válido', async () => {
  let calls = 0;
  const service = new GitAiCodeReviewService(
    {
      review: async (_model, messages) => {
...
test('cancelamento durante a síntese global preserva a revisão parcial', async () => {
  let calls = 0;
