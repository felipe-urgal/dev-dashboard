...
test('rotas do gateway de servidor de linguagem (status por kind e opt-in Rails runtime)', async (context) => {
  const fixtureRoot = await mkdtemp(
    path.join(tmpdir(), 'dev-dashboard-lsp-routes-'),
  );
  ...
  await context.test(
    'status javascript-typescript não suportado para projeto Rails',
    async () => {
  ...
  await context.test(
    'status ruby reconhece Gemfile e reporta add-on Rails resolvido',
    async () => {
      const response = await app.inject({
        method: 'GET',
  ...
  await context.test(
    'habilitar Rails runtime sem token de confirmação é recusado',
    async () => {
      const response = await app.inject({
        method: 'POST',
  ...
  await context.test(
    'fluxo completo: preparar confirmação e habilitar Rails runtime',
    async () => {
