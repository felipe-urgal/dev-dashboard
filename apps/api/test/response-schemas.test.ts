...
test('response schemas strip properties not part of the public contract', async (context) => {
  ...
  await context.test(
    ...
    async () => {
      ...
      assert.equal(body.workspaces[0]?.id, pollutedWorkspaceId);
      assert.equal(body.workspaces[0]?.internalDebugNote, undefined);
    },
  );
  ...
  await context.test(
    ...
    async () => {
      ...
      assert.equal(response.statusCode, 200);
      assert.equal(body.projects[0]?.id, pollutedProjectId);
      assert.equal(body.projects[0]?.internalDebugNote, undefined);
    },
  );
  ...
  await context.test(
    ...
    async () => {
      await writeFile(
        path.join(processDirectory, processStateFileName(pollutedProjectId)),
        JSON.stringify({
          id: 'proc-status',
      ...
      assert.equal(body.process?.internalDebugNote, undefined);
    },
  );
  ...
  await context.test(
    ...
    async () => {
      ...
      await writeFile(
        path.join(processDirectory, processStateFileName(stoppableProjectId)),
        JSON.stringify({
          id: 'proc-stop',
