...
test('histórico de mutações Git: excluir branch local (branch-delete)', async (context) => {
  const fixtureRoot = await mkdtemp(
    path.join(tmpdir(), 'dev-dashboard-git-delete-history-'),
  );
  ...
  await context.test(
    'excluir branch com sucesso é registrado com risco destructive',
    async () => {
      const confirmationResponse = await app.inject({
        method: 'POST',
        url: '/api/projects/p1/git/branches/delete/confirmations',
        headers,
      ...
      const deleteResponse = await app.inject({
        method: 'POST',
        url: '/api/projects/p1/git/branches/delete',
        headers,
