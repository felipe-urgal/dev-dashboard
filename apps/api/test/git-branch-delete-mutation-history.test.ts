...
/**
 * Verifica que `GitBranchDeleteService` (`branch-delete`), migrado da task
 * 098, grava eventos no histórico compartilhado no mesmo padrão de
 * `git-mutation-history-routes.test.ts`.
 */
test('histórico de mutações Git: excluir branch local (branch-delete)', async (context) => {
  const fixtureRoot = await mkdtemp(path.join(tmpdir(), 'dev-dashboard-git-delete-history-'));
  ...
  await git(repoPath, ['branch', 'feature/doomed']);
  ...
  await context.test('excluir branch com sucesso é registrado com risco destructive', async () => {
    const confirmationResponse = await app.inject({
      method: 'POST', url: '/api/projects/p1/git/branches/delete/confirmations', headers,
      payload: JSON.stringify({ branch: 'feature/doomed' }),
    });
    const { confirmation } = confirmationResponse.json<ConfirmationResponse>();
    const deleteResponse = await app.inject({
      method: 'POST', url: '/api/projects/p1/git/branches/delete', headers,
      payload: JSON.stringify({ branch: 'feature/doomed', confirmationToken: confirmation.token }),
    });
    ...
    assert.equal(body.events[0]!.operationId, 'branch-delete');
  ...
  await context.test('confirmação ausente/expirada não gera evento de histórico', async () => {
    ...
    const response = await app.inject({
      method: 'POST', url: '/api/projects/p1/git/branches/delete', headers,
      payload: JSON.stringify({ branch: 'main', confirmationToken: 'z'.repeat(64) }),
    });
  ...
  await context.test('falha controlada (branch inexistente) é registrada com o código de erro', async () => {
    ...
    assert.ok(failure);
    assert.equal(failure!.operationId, 'branch-delete');
