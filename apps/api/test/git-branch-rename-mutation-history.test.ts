...
interface ConfirmationResponse {
  confirmation: { token: string; operation: string; currentName: string; nextName: string; expiresAt: string };
}
...
test('histórico de mutações Git: renomear branch (branch-rename)', async (context) => {
  const fixtureRoot = await mkdtemp(path.join(tmpdir(), 'dev-dashboard-git-rename-history-'));
  ...
  await context.test('renomear com sucesso é registrado com risco write-safe', async () => {
    const confirmationResponse = await app.inject({
      method: 'POST', url: '/api/projects/p1/git/branches/rename/confirmations', headers,
      payload: JSON.stringify({ currentName: 'feature/old', nextName: 'feature/new' }),
    });
    const { confirmation } = confirmationResponse.json<ConfirmationResponse>();
    const renameResponse = await app.inject({
      method: 'POST', url: '/api/projects/p1/git/branches/rename', headers,
      payload: JSON.stringify({ currentName: 'feature/old', nextName: 'feature/new', confirmationToken: confirmation.token }),
    });
  ...
  await context.test('confirmação ausente/expirada não gera evento de histórico', async () => {
    ...
    const response = await app.inject({
      method: 'POST', url: '/api/projects/p1/git/branches/rename', headers,
      payload: JSON.stringify({ currentName: 'feature/new', nextName: 'feature/other', confirmationToken: 'z'.repeat(64) }),
    });
  ...
  await context.test('falha controlada (branch inexistente) é registrada com o código de erro', async () => {
    const confirmationResponse = await app.inject({
      method: 'POST', url: '/api/projects/p1/git/branches/rename/confirmations', headers,
      payload: JSON.stringify({ currentName: 'no-such-branch', nextName: 'renamed' }),
    });
    const { confirmation } = confirmationResponse.json<ConfirmationResponse>();
    const response = await app.inject({
      method: 'POST', url: '/api/projects/p1/git/branches/rename', headers,
      payload: JSON.stringify({ currentName: 'no-such-branch', nextName: 'renamed', confirmationToken: confirmation.token }),
    });
