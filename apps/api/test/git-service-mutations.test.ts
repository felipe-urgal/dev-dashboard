...
test('createBranch com nome inválido falha antes de tocar no repositório', async (context) => {
  ...
  await assert.rejects(
    ...
    (error: unknown) =>
      error instanceof GitMutationError && error.code === 'GIT_BRANCH_INVALID',
  );
  await assert.rejects(
...
test('createBranch em branch já existente falha com GIT_BRANCH_EXISTS', async (context) => {
  ...
  const first = service.prepareMutationConfirmation(
    'p1',
    'create-branch',
    'feature/x',
  );
  ...
  const second = service2.prepareMutationConfirmation(
    'p1',
    'create-branch',
    'feature/x',
  );
  await assert.rejects(
    ...
    (error: unknown) =>
      error instanceof GitMutationError && error.code === 'GIT_BRANCH_EXISTS',
  );
});
...
test('switchBranch em árvore suja falha com GIT_WORKING_TREE_DIRTY', async (context) => {
  ...
  const service = new GitService();
  const created = service.prepareMutationConfirmation(
    'p1',
    'create-branch',
    'feature/z',
  );
...
test('push rejeitado pelo remoto quando o clone está desatualizado falha com GIT_PUSH_REJECTED', async (context) => {
  ...
  await assert.rejects(
    ...
    (error: unknown) =>
      error instanceof GitMutationError && error.code === 'GIT_PUSH_REJECTED',
  );
});
...
test('push em HEAD destacado falha com GIT_DETACHED_HEAD', async (context) => {
  ...
  await assert.rejects(
    ...
    (error: unknown) =>
      error instanceof GitMutationError && error.code === 'GIT_DETACHED_HEAD',
  );
});
...
test('pull avança em fast-forward após confirmação', async (context) => {
  ...
  assert.equal(result.branch, 'main');
  const content = await execFileAsync('git', ['show', 'HEAD:README.md'], {
    cwd: secondClone,
  });
...
test('pull sem upstream falha com GIT_NO_UPSTREAM', async (context) => {
  ...
  await assert.rejects(
    ...
    (error: unknown) =>
      error instanceof GitMutationError && error.code === 'GIT_NO_UPSTREAM',
  );
});
...
test('pull com histórico divergente falha com GIT_PULL_DIVERGED', async (context) => {
  ...
  await assert.rejects(
    ...
    (error: unknown) =>
      error instanceof GitMutationError && error.code === 'GIT_PULL_DIVERGED',
  );
});
...
test('amend substitui a mensagem do último commit sem exigir alterações', async (context) => {
  ...
  const previousHead = await execFileAsync('git', ['rev-parse', 'HEAD'], {
    cwd: root,
  });
  const service = new GitService();
  const confirmation = service.prepareMutationConfirmation(
...
test('commit com mensagem vazia falha com GIT_COMMIT_MESSAGE_INVALID antes de tocar no repositório', async (context) => {
  ...
  await assert.rejects(
    ...
    (error: unknown) =>
      error instanceof GitMutationError &&
      error.code === 'GIT_COMMIT_MESSAGE_INVALID',
  );
});
...
test('save prepara todas as alterações (incluindo não rastreadas) e aplica o prefixo do branch', async (context) => {
  ...
  assert.equal(
    status.stdout.trim(),
    '',
    'a árvore de trabalho deveria ficar limpa após o save',
  );
...
test('save sem confirmação é recusado antes de preparar qualquer arquivo', async (context) => {
  ...
  assert.match(
    status.stdout,
    /\?\? novo\.txt/,
    'o arquivo não deveria ter sido staged',
  );
});
