...
test('getFileDiff mascara segredos e sinaliza truncamento', async (context) => {
  ...
  const bigContent =
    'password="s3cret1234567890abcdef"\n' + 'x'.repeat(300_000);
  ...
  assert.ok(
    !diff.content.includes('s3cret1234567890abcdef'),
    'segredo não pode aparecer no conteúdo mascarado',
  );
});
...
test('getFileDiff rejeita path fora do projeto', async (context) => {
  ...
  await assert.rejects(
    ...
    (error: unknown) =>
      ...
      error.code === 'GIT_DIFF_PATH_OUTSIDE_PROJECT',
  );
  await assert.rejects(
    () => service.getFileDiff(root, '/etc/passwd', 'combined'),
    (error: unknown) =>
  ...
  await assert.rejects(
    ...
    (error: unknown) =>
      error instanceof GitDiffError && error.code === 'GIT_DIFF_PATH_INVALID',
  );
});
...
test('getDiffSnapshot em repositório sem commits compara contra a árvore vazia', async (context) => {
  ...
  assert.ok(
    paths.includes('first.md'),
    `esperava first.md no snapshot, obteve ${JSON.stringify(paths)}`,
  );
...
test('getDiffSnapshot detecta renomes e preserva previousPath', async (context) => {
  ...
  assert.ok(
    renamed,
    `esperava src/renamed.ts em ${JSON.stringify(snapshot.files.map((f) => f.path))}`,
  );
...
test('getFileLines devolve a faixa pedida do arquivo alterado', async (context) => {
  ...
  const lines = Array.from(
    { length: 40 },
    (_unused, index) => `linha ${index + 1}`,
  );
  await writeFile(path.join(root, 'src', 'app.ts'), `${lines.join('\n')}\n`);

  const service = new GitService();
  const result = await service.getFileLines(
    root,
    'src/app.ts',
    'combined',
    5,
...
test('getFileLines lê o blob do índice quando o escopo é index', async (context) => {
  ...
  const worktree = await service.getFileLines(
    root,
    'src/app.ts',
    'combined',
    1,
...
test('getFileLines recusa arquivo que não está no diff do escopo', async (context) => {
  ...
  await assert.rejects(
    ...
    (error: unknown) =>
      error instanceof GitDiffError &&
      error.code === 'GIT_DIFF_PATH_NOT_IN_DIFF',
  );
});
...
test('getFileLines recusa faixa invertida ou grande demais', async (context) => {
  ...
  await assert.rejects(
    ...
    (error: unknown) =>
      error instanceof GitDiffError && error.code === 'GIT_DIFF_RANGE_INVALID',
  );
  await assert.rejects(
