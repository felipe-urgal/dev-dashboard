...
test('histórico de mutações Git: desfazer (undo-commit/undo-file)', async (context) => {
  const fixtureRoot = await mkdtemp(path.join(tmpdir(), 'dev-dashboard-git-undo-history-'));
  ...
  await context.test('desfazer o último commit com sucesso é registrado com risco destructive', async () => {
    const confirmationResponse = await app.inject({
      method: 'POST', url: '/api/projects/p1/git/undo/confirmations', headers,
    ...
    const undoResponse = await app.inject({
      method: 'POST', url: '/api/projects/p1/git/undo/commit', headers,
  ...
  await context.test('confirmação ausente/expirada não gera evento de histórico', async () => {
    ...
    const response = await app.inject({
      method: 'POST', url: '/api/projects/p1/git/undo/file', headers,
      payload: JSON.stringify({ path: 'README.md', confirmationToken: 'z'.repeat(64) }),
    });
  ...
  await context.test('falha controlada (arquivo sem alterações) é registrada com o código de erro', async () => {
    const confirmationResponse = await app.inject({
      method: 'POST', url: '/api/projects/p1/git/undo/confirmations', headers,
    ...
    const response = await app.inject({
      method: 'POST', url: '/api/projects/p1/git/undo/file', headers,
      payload: JSON.stringify({ path: 'README.md', confirmationToken: confirmation.token }),
    });
    ...
    const historyResponse = await app.inject({ method: 'GET', url: '/api/projects/p1/git/mutation-history', headers });
    const body = historyResponse.json<HistoryResponse>();
    const failure = body.events.find((event) => event.operationId === 'undo-file');
    assert.ok(failure);
