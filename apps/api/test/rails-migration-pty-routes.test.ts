...
interface StatusResponse {
  snapshot: { operation: string; status: string } | null;
}
...
test('rotas de migration PTY respeitam a instância de ambiente', async (context) => {
  const fixtureRoot = await mkdtemp(
    path.join(tmpdir(), 'dev-dashboard-rails-migration-pty-routes-'),
  );
  ...
  await context.test(
    ...
    async () => {
      const response = await app.inject({
        method: 'GET',
        url:
          '/api/projects/p1/rails/migrations/pty/status?environmentInstanceId=' +
          encodeURIComponent(WORKTREE_ENVIRONMENT_ID),
        headers,
      });
  ...
  await context.test(
    ...
    async () => {
      const response = await app.inject({
        method: 'GET',
        url:
          '/api/projects/p1/rails/migrations/pty/status?' +
          'environmentInstanceId=environment%3Aworktree%3Ap1%3Amissing',
        headers,
      });
