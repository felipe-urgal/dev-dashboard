...
test.describe('Mutações de branch Git do projeto', () => {
  test('cria, troca e recusa nome duplicado; projeto sem Git fica vazio', async ({ page }) => {
    ...
    await expect(page.getByText('Este projeto não é um repositório Git.')).toBeVisible();
    ...
    await expect(
      page.locator('.branch-table-row', { has: page.getByText('main', { exact: true }) }),
    ).toBeVisible();
    ...
    await expect(page.getByRole('button', { name: 'Criar branch' })).toBeVisible();
    await page.getByRole('button', { name: 'Criar branch' }).click();
    await expect(page.getByText('Branch "feature/e2e-branch" criada e selecionada.')).toBeVisible();
    ...
    const mainRow = page.locator('.branch-table-row', { has: page.getByText('main', { exact: true }) });
    await mainRow.getByRole('button', { name: 'Trocar' }).click();
    await expect(page.getByRole('button', { name: 'Trocar de branch' })).toBeVisible();
