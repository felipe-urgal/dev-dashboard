...
test.describe('Catálogo de scripts do projeto', () => {
  test('executa com sucesso, falha sob confirmação e reseta ao trocar de projeto', async ({ page }) => {
    ...
    await expect(page.getByRole('tab', { name: 'Execuções' })).toHaveAttribute('aria-selected', 'true');
    await expect(page.getByText('Em execução', { exact: true }).first()).toBeVisible();
    await expect(page.getByText('Concluída', { exact: true }).first()).toBeVisible({ timeout: 10_000 });
    ...
    await expect(page.getByText('Em execução', { exact: true }).first()).toBeVisible();
    await expect(page.getByText('Falhou', { exact: true }).first()).toBeVisible({ timeout: 10_000 });
