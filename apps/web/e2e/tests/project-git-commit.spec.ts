...
test.describe('Commit do projeto', () => {
  test('vazio sem alterações, cria commit com sucesso e reseta ao trocar de projeto', async ({ page }) => {
    ...
    const lockfilePath = path.join(info.workspaceDirectory, 'sample-node-app', 'package-lock.json');
    ...
    await page.getByLabel('Mensagem do commit').fill('chore: ajusta lockfile via e2e');
    await submitButton.click();
    const confirmDialog = page.getByRole('alertdialog');
    await expect(confirmDialog.getByRole('button', { name: 'Criar commit' })).toBeVisible();
    await confirmDialog.getByRole('button', { name: 'Criar commit' }).click();
    await expect(page.getByText(/^Commit ".+" criado: chore: ajusta lockfile via e2e$/)).toBeVisible();
    await expect(page.getByText('0 alterações rastreadas')).toBeVisible();
