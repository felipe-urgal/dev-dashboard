...
function page(
  overrides: Partial<GitMutationHistoryPage> = {},
...
describe('ProjectGitMutationHistoryPage', () => {
  it('mostra estado vazio quando não há eventos', async () => {
    ...
    const wrapper = mount(ProjectGitMutationHistoryPage, {
      props: { project },
    });
    await flushPromises();
  ...
  it('mostra rótulo do catálogo, risco e resultado de cada evento', async () => {
    fetchProjectGitMutationHistory.mockResolvedValueOnce(
      page({
        total: 1,
        totalPages: 1,
        events: [
  ...
  it('busca a próxima página ao clicar em "Próxima"', async () => {
    ...
    expect(fetchProjectGitMutationHistory).toHaveBeenLastCalledWith(
      'p1',
      2,
      10,
    );
