...
test('monta cabeçalho com truncamento e mascaramento preservando o snapshot', () => {
  ...
  assert.ok(prepared);
  assert.match(prepared.filename, /^dev-dashboard-API-principal-testes-execucao-42-/);
  ...
  assert.match(prepared.content, /Conteúdo mascarado: Sim \(1 substituição\(ões\)\)/);
...
test('recusa snapshot vazio', () => {
  assert.equal(prepareLogExport({
    projectName: 'Projeto',
    origin: 'servidor',
    snapshot: { content: '  \n', truncated: false, masked: false, redactionCount: 0 },
  }), null);
});
