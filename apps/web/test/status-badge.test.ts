...
test('StatusBadge renderiza o slot com a classe de tone e tamanho padrão sm', () => {
  const wrapper = mount(StatusBadge, { props: { tone: 'success' }, slots: { default: 'ok' } });
...
test('StatusBadge respeita size md e cobre todos os tones', () => {
  ...
  for (const tone of tones) {
    const wrapper = mount(StatusBadge, { props: { tone, size: 'md' }, slots: { default: tone } });
    const root = wrapper.get('.dd-status-badge');
    assert.equal(root.classes().includes(`dd-status-badge-${tone}`), true, `tone ${tone}`);
