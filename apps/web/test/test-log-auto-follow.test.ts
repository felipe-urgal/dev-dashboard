...
test('considera o log no final usando uma margem confortável', () => {
  assert.equal(isTestLogNearEnd({ scrollTop: 340, scrollHeight: 500, clientHeight: 120 }), true);
  assert.equal(isTestLogNearEnd({ scrollTop: 100, scrollHeight: 500, clientHeight: 120 }), false);
});
