...
function navigationItem(
  ...
  return {
    ...
    mode,
    kind: 'navigation',
    searchText: normalizePaletteText(`${label} ${description}`),
...
function actionItem(
  ...
  return {
    ...
    mode: 'action',
    kind: 'action',
    searchText: normalizePaletteText(`${label} ${description} ${aliases}`),
  };
}
...
export function useCommandPaletteItems(options: UseCommandPaletteItemsOptions) {
  function buildProjectItems(project: Project): PaletteItem[] {
    ...
    const areas: NavigationPaletteItem[] = [
      navigationItem(
        `area-project-details-${project.id}`,
        'Projeto atual',
      ...
      navigationItem(
        `area-project-server-${project.id}`,
        'Projeto atual',
        'Servidor',
        'Configurar e controlar o servidor',
        { name: 'project-server', params },
        ServerStackIcon,
        'page',
      ),
      navigationItem(
        `area-project-logs-${project.id}`,
        'Projeto atual',
        'Logs',
        'Acompanhar a saída do servidor',
        { name: 'project-logs', params },
        DocumentTextIcon,
        'page',
      ),
      navigationItem(
        `area-project-git-${project.id}`,
        'Projeto atual',
        'Git',
        'Branches, diff, commit e sincronização',
        { name: 'project-git', params },
        CodeBracketIcon,
        'page',
      ),
      navigationItem(
        `area-project-tests-${project.id}`,
        'Projeto atual',
        'Testes',
      ...
      navigationItem(
        ...
        'Projeto atual',
        'Scripts',
        'Catálogo autorizado do projeto',
        { name: 'project-scripts', params },
        CommandLineIcon,
        'page',
      ),
    ];
    const shortcuts: NavigationPaletteItem[] = [
      ...(project.capabilities.includes('server')
        ? [
            navigationItem(
              `command-server-${project.id}`,
              'Comandos do projeto',
              'Abrir servidor',
              'Configuração e status do servidor',
              { name: 'project-server', params },
              ServerStackIcon,
              'action',
              undefined,
      ...
      ...(project.capabilities.includes('git')
        ? [
            navigationItem(
              `command-git-${project.id}`,
              'Comandos do projeto',
              'Abrir Git',
              'Branches, diff e histórico',
              { name: 'project-git', params },
              CodeBracketIcon,
              'action',
              undefined,
            ...
            navigationItem(
              `command-git-sync-${project.id}`,
              'Comandos do projeto',
              'Sincronizar main',
              'Abrir sincronização segura do repositório',
              { name: 'project-git', params, query: { tab: 'sync' } },
              ArrowPathIcon,
              'action',
              undefined,
              'Abrir',
            ...
            navigationItem(
              `command-git-branch-${project.id}`,
              'Comandos do projeto',
              'Criar branch',
              'Abrir gerenciamento de branches',
              { name: 'project-git', params, query: { tab: 'branches' } },
              CodeBracketIcon,
              'action',
              undefined,
              'Abrir',
            ...
            navigationItem(
              `command-git-commit-${project.id}`,
              'Comandos do projeto',
              'Commitar alterações',
              'Abrir criação e correção de commit',
              { name: 'project-git', params, query: { tab: 'commit' } },
              CodeBracketIcon,
              'action',
              undefined,
              'Abrir',
      ...
      ...(project.capabilities.includes('database')
        ...
        : []),
      ...(project.capabilities.includes('scripts')
        ? [
            navigationItem(
              `command-scripts-${project.id}`,
              'Comandos do projeto',
              'Abrir scripts',
              'Catálogo autorizado do projeto',
              { name: 'project-scripts', params },
    ...
    if (
      ...
      if (status === 'running' || status === 'starting') {
        actions.push(
          actionItem(
            `${project.id}:server-stop`,
            'Parar servidor',
            `Interromper o servidor de ${project.name}`,
            StopCircleIcon,
            'atencao',
            { type: 'server-stop' },
        ...
        actions.push(
          actionItem(
            `${project.id}:server-start`,
            'Iniciar servidor',
            `Executar o servidor de ${project.name}`,
            ServerStackIcon,
            'reversivel',
            { type: 'server-start' },
            'start server server start iniciar server',
          ),
        );
      }
    }
    if (
      loadedProjectId.value === project.id &&
      project.capabilities.includes('tests') &&
      testOverview.value?.supported
    ) {
      const status = testProcess.value?.status;
      if (status === 'running' || status === 'starting') {
        actions.push(
          actionItem(
            `${project.id}:test-stop`,
            'Parar testes',
            'Interromper a suíte em execução',
            StopCircleIcon,
            'atencao',
            { type: 'test-stop' },
          ),
        ...
        for (const command of testOverview.value.commands) {
          actions.push(
            actionItem(
              ...
              BeakerIcon,
              'reversivel',
              { type: 'test-start', commandId: command.id },
            ),
          );
        }
      }
    ...
    if (
      ...
      for (const script of scriptCatalog.value?.items.filter(
        (item) =>
          ...
          isRunnableProjectScript(item, project),
      ) ?? []) {
        actions.push(
          actionItem(
            `${project.id}:script-${script.id}`,
            script.name,
            script.description || script.command,
            CommandLineIcon,
            script.risk === 'read-only' ? 'reversivel' : 'atencao',
            { type: 'script-start', script },
          ),
        );
      }
    }
  ...
  const items = computed<PaletteItem[]>(() => {
    ...
    const projectItems: PaletteItem[] = project
      ? buildProjectItems(project)
      : [];
    const globalItems: NavigationPaletteItem[] = [
      navigationItem(
        ...
        'Páginas',
        'Visão geral',
        'Dashboard e repositórios',
        { name: 'dashboard', hash: '#overview' },
        HomeIcon,
        'page',
      ),
      navigationItem(
      ...
      navigationItem(
        ...
        'Páginas',
        'Configurações',
        'Preferências e retenção local',
        { name: 'settings' },
        Cog6ToothIcon,
        'page',
      ),
    ];
    const workspaceItems = options
      .workspaces()
      .map((workspace) =>
        navigationItem(
    ...
    const allProjectItems = [...options.projects()]
      ...
      .map((item) => ({
        ...navigationItem(
          ...
          item.name,
          item.path,
          { name: 'project-details', params: { projectId: item.id } },
          FolderIcon,
          'project',
        ),
        projectId: item.id,
  ...
  const orderedItems = computed<PaletteItem[]>(() => {
    ...
    const candidates = items.value.filter(
      (item) => parsed.mode === 'all' || item.mode === parsed.mode,
    );
    if (parsed.value) {
      return candidates
        .map((item) => ({
          item,
          score: paletteFuzzyScore(item.searchText, parsed.value),
        }))
  ...
  const groupViews = computed<PaletteGroupView[]>(() => {
    ...
    const recentSet = !parsed.value
      ? new Set(
          options.recentIds.value.filter((id) =>
            orderedItems.value.some((item) => item.id === id),
          ),
        )
      : new Set<string>();
    const result: PaletteGroupView[] = [];
    for (const item of orderedItems.value) {
      const name: PaletteGroup = recentSet.has(item.id)
        ? 'Recentes'
        : item.group;
