...
export function isTestLogErrorLine(value: string): boolean {
  ...
  if (
    ...
    return true;
  if (/\b(?:test|suite|build|command|runner)\s+(?:has\s+)?failed\b/i.test(line))
    return true;
...
export function classifyTestLogLine(value: string): TestLogVisualTone | null {
  ...
  if (
    /^(?:\$|>\s|yarn(?:\s+run)?\b|npm(?:\s+run)?\b|pnpm\b|bun\b|bundle exec\b|bin\/rails\b|rails test\b|ruby\b)/i.test(
      line,
    )
  ) {
    return 'command';
