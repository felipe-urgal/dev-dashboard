...
const selectedProject = computed(() => {
  ...
  const exact = props.projects.find((project) =>
    [project.id, project.name].some(
      (value) => normalizePaletteText(value) === projectQuery,
    ),
  );
...
function completeActiveItem(): boolean {
  ...
  if (parsedQuery.value.mode !== 'action' || !selectedProject.value)
    return false;
  const item = orderedItems.value[activeIndex.value];
...
async function select(item: PaletteItem): Promise<void> {
  ...
  try {
    ...
    pendingActionId.value = undefined;
    query.value =
      parsedQuery.value.project !== undefined ? `@${project.name} > ` : '';
    activeIndex.value = 0;
  } catch (error) {
    feedback.value =
...
function itemHint(item: PaletteItem): string {
  ...
  if (
    parsedQuery.value.mode === 'project' &&
    item.kind === 'navigation' &&
    item.projectId
  )
    return 'Tab para usar';
  if (parsedQuery.value.mode === 'action')
    return item.kind === 'action' && item.risk === 'atencao'
      ? 'Ação sensível'
      : 'Tab completa';
...
watch(
  ...
  (projectId, previousProjectId) => {
    if (
      !open.value ||
      parsedQuery.value.project === undefined ||
      projectId === previousProjectId
    )
      return;
    void loadProjectActions(selectedProject.value).then((result) => {
