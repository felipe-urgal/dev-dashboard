...
export function describeSelect(statement: string): SqlExplanation {
  ...
  const returnsFullRecords = /^(?:DISTINCT\s+)?(?:\*|(?:`?\w+`?\.)?\*)$/i.test(
    projection,
  );
  ...
  if (isCount) {
    ...
    expectedReturn =
      'Um valor verdadeiro ou falso indicando se encontrou algum registro.';
    ...
    const fields = projection
      ...
      .slice(0, 4);
    const suffix =
      projection.split(',').length > fields.length ? ' e outros campos' : '';
  ...
  if (relatedTables.length) {
    details.push(
      `cruzando dados relacionados de ${relatedTables.map(code).join(', ')}`,
    );
  }
  if (hasSoftDeleteFilter(statement)) {
    details.push(
      'considerando apenas registros que não foram excluídos logicamente',
    );
...
export function explainSql(statement: string): SqlExplanation {
  ...
  const operation = normalized
    .match(/^(SELECT|INSERT|UPDATE|DELETE|BEGIN|COMMIT|ROLLBACK)\b/i)?.[1]
    ?.toUpperCase();
  const mainTable = extractMainTable(normalized);
  const relatedTables = extractJoinedTables(normalized);
  const filtered = hasWhere(normalized)
    ? ' que atendem aos filtros da consulta'
    : '';
  ...
  switch (operation) {
    ...
    case 'INSERT':
      return {
        ...
        expectedReturn:
          'Normalmente retorna a confirmação da inserção e, dependendo do banco, o identificador criado.',
        mainTable,
        relatedTables,
      };
    case 'UPDATE':
      return {
        ...
        expectedReturn:
          'Normalmente retorna a quantidade de registros alterados, não os registros completos.',
        mainTable,
        relatedTables,
      };
    case 'DELETE':
      return {
        ...
        expectedReturn:
          'Normalmente retorna a quantidade de registros removidos.',
        mainTable,
        relatedTables,
      };
    case 'BEGIN':
      return {
        ...
        expectedReturn:
          'Não retorna registros; apenas abre o bloco transacional.',
        relatedTables,
      };
    case 'COMMIT':
      return {
        ...
        expectedReturn:
          'Não retorna registros; confirma que as mudanças foram persistidas.',
        relatedTables,
      };
    case 'ROLLBACK':
      return {
        ...
        expectedReturn:
          'Não retorna registros; informa que as mudanças foram revertidas.',
        relatedTables,
      };
    default:
      return {
