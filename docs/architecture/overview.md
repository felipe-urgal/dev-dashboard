...
- rodar comandos de execução única (suíte completa de testes e operações de Migration Rails) num PTY destacável que sobrevive à desconexão (`apps/api/src/services/detachable-execution-service.ts`, `project-test-pty-service.ts`, `rails-migration-pty-service.ts` — task 234, ver `docs/guia/testes.md` e `docs/guia/banco-de-dados.md`);
