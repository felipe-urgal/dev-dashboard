...
O servidor gerenciado, os Rails workers (Sidekiq/Webpack) e as execuções legadas de testes persistem estado e log por `projectId + environmentInstanceId + kind`; status, logs, stop e subscriptions resolvem a mesma identidade. O servidor também mantém health na mesma instance. Detecção e resolução de comando dos workers usam o `cwd` do `ExecutionContext`, portanto um binstub/dependência presente apenas no linked worktree não é inferido a partir do checkout principal. Isso permite que `primary` e linked worktrees mantenham processos independentes no mesmo projeto. Estados legados sem `environmentInstanceId` continuam sendo aceitos somente como compatibilidade da instância `primary`, evitando atribuir silenciosamente ownership legado a um worktree.
