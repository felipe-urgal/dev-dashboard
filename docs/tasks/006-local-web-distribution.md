...
2. Abrir `http://127.0.0.1:4343` e confirmar o carregamento.
...
Foram alterados a configuração/inicialização da API, segurança local, cliente HTTP web, diagnóstico e documentação; foram criados o plugin estático, configuração testável, orquestrador e testes correspondentes. A sessão vive somente em memória do navegador e expira; reiniciar a API não revoga cookies já assinados enquanto o token persistente e o prazo forem válidos. O modelo continua mono usuário e não protege contra outro processo executado pelo mesmo usuário que consiga ler o token.
