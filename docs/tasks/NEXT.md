...
recebeu uma sequência de entregas (054–058: limpeza de Rotas/Dependências,
...
e geração de model/migration) — vale revisitar a aba como um todo antes de
propor a próxima fatia, em vez de assumir de antemão qual é a lacuna mais
importante.

Candidatos observados ao longo dessas entregas, nenhum dimensionado ainda:

- os arquivos criados por `rails generate` (058) aparecem só como texto; não
...
- `runMigrationMutation` (migrate/rollback/seed/db:prepare) sempre opera em
  ...
  num banco secundário pela interface (057);
- a correlação entre bloco de `db:migrate:status` e banco configurado (057)
