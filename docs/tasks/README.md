...
- `094-environment-profiles.md` — perfis de ambiente reutilizáveis na tela de Configurações: `EnvironmentProfileRepository` em `packages/core`, rotas `/api/settings/environment-profiles`, e formulário na UI que nunca envia valor para variáveis de nome sensível (token, senha, chave, credencial); aplicar um perfil às variáveis declaradas de um script fica para uma próxima entrega.
- `095-rails-sidekiq-webpack-credentials.md` — operações de Sidekiq e webpack-dev-server no dashboard web via dois novos `kind` de processo gerenciado (`worker`, `webpack`) em `packages/process-manager`, catálogo fechado por `workerId` (nunca comando/caminho vindo do navegador) e status somente leitura de credenciais Rails (existência de `config/credentials.yml.enc`/`master.key`, sem ler conteúdo); editar credenciais criptografadas fica fora de escopo.
