...
async function fixture(t) {
  ...
  await writeFile(path.join(root, 'docs', 'site', 'index.html'), '<!doctype html><title>Docs</title>');
  await writeFile(path.join(root, 'docs', 'index.md'), '# Documentação\n\nVisão profissional do projeto.\n');
  await writeFile(path.join(root, 'docs', 'architecture', 'overview.md'), '# Arquitetura\n\nFluxo seguro da API Fastify.\n\n## Camadas\n');
...
test('cataloga documentos e preserva a ordem principal', async (t) => {
  ...
  assert.ok(catalog.pages.some((page) => page.path === 'docs/architecture/overview.md'));
  assert.equal(catalog.pages.find((page) => page.path === 'docs/architecture/overview.md')?.group, 'Arquitetura');
});

test('rejeita traversal e arquivos fora do catálogo', async (t) => {
  const root = await fixture(t);
  const catalog = await buildCatalog(root);
  await assert.rejects(() => resolveDocumentPath(root, '../segredo.md', catalog), /fora do escopo/);
  await assert.rejects(() => resolveDocumentPath(root, 'docs/inexistente.md', catalog), /não catalogado/);
});
...
test('expõe health, catálogo, conteúdo e busca pela API local', async (t) => {
  ...
  const catalog = await fetch(`${address.url}/api/docs`).then((response) => response.json());
  assert.equal(catalog.defaultDocument, 'docs/index.md');

  const contentResponse = await fetch(`${address.url}/api/docs/content?path=${encodeURIComponent('docs/index.md')}`);
  assert.equal(contentResponse.status, 200);
  const content = await contentResponse.json();
  assert.match(content.markdown, /Visão profissional/);

  const search = await fetch(`${address.url}/api/search?q=seguro`).then((response) => response.json());
