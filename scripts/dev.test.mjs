...
import { getProcessDefinitions, stopChild } from './dev.mjs';
...
test('stopChild loga outros erros sem lançar', () => {
  ...
  assert.ok(
    messages.some((m) => m.includes('Falha ao encerrar')),
    `Expected error log, got: ${messages}`,
  );
});
