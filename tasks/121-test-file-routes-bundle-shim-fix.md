# Task 121 — Corrige causa real do CI vermelho no PR #232 (shim de `bundle` ausente)

## Contexto

O PR #232 (tasks 118/119) continuou vermelho na CI mesmo depois da task 120
(alargar margem dos limiares de cobertura). A task 120 diagnosticou errado:
...
task 119): dois testes do bloco "rotas de caso específico de teste (RSpec)"
...
O teste da task 119 inicia de verdade a execução de um caso RSpec via
...
task 120 não conseguiu reproduzir a falha localmente em nenhuma versão de
...
novo da task 119 não seguiu esse padrão.
...
## O que a task 120 acertou e o que não

O alargamento de margem dos limiares de cobertura (task 120) continua
...
não foi revertido. Mas o diagnóstico da task 120 (de que a causa raiz da
